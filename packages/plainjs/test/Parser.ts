...
import { default as Parser } from '../src/Parser.js';

async function parseInput<TRaw extends object, T extends object>(parser: Parser<TRaw, T>, nodeStream: TRaw): Promise<string> {
  return parser.parse(nodeStream);
}

export default function (jsonFixtures: Record<string, () => any>, csvFixtures: Record<string, any>) {
  const testRunner = new TestRunner('Parser');

  testRunner.add(
    'should not modify the JSON object passed passed',
    async (t) => {
      ...
      const originalJson = JSON.parse(
        JSON.stringify(jsonFixtures['unwindComplexObject']())
      );
